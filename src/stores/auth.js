  ...
  actions: {
    async login(credentials) {
      ...
      try {
        ...
        const executeLoginAttempt = async (url) => {
          try {
        ...
        const docenteResponse = await executeLoginAttempt('/docentes/login/email')

        if (docenteResponse.status === 200 && docenteResponse.data?.success) {
        ...
        if (docenteResponse.status !== 401 && docenteResponse.status !== 400) {
          return {
            success: false,
            error: docenteResponse.data?.message || 'Error al autenticar docente'
          }
        }

        const profesorResponse = await executeLoginAttempt('/profesores/login/correo')

        if (profesorResponse.status === 200 && profesorResponse.data?.success) {
        ...
        if (profesorResponse.status === 401 || profesorResponse.status === 400) {
          return {
            success: false,
        ...
        return {
          success: false,
          error: profesorResponse.data?.message || 'Error al autenticar profesor'
        }
      } catch (error) {
        return {
          success: false,
