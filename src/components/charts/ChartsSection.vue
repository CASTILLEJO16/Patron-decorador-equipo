<template>
  <div class="charts-section">
    ...
    <div class="charts-grid">
      ...
      <!-- Gráfico de Pastel con Patrón Decorador -->
      <div class="chart-card">
        <div class="chart-header">
          <h3>Distribución por Estado Académico (Decorador)</h3>
        ...
        <div class="chart-container">
          <DecoratedPieChart 
            ...
            :options="{ title: 'Distribución por Estado Académico' }"
            :show-controls="true"
            :show-filter-controls="true"
            :show-export-controls="true"
            :show-info="true"
            initial-type="pie"
            initial-theme="light"
            initial-animation="bounce"
            @chart-ready="handleDecoratedChartReady"
            @export-complete="handleExportComplete"
            @error="handleDecoratedChartError"
          />
        </div>
      </div>

      <!-- Gráfico de Pastel Original (para comparación) -->
      <div class="chart-card">
        <div class="chart-header">
          <h3>Distribución por Estado Académico (Original)</h3>
          <select v-model="pieChartSemesterOriginal" class="period-select">
            <option value="">Todos los Semestres</option>
            <option v-for="sem in availableSemesters" :key="sem" :value="sem">
              Semestre {{ sem }}
            </option>
...
import DecoratedPieChart from './DecoratedPieChart.vue'
...
// Manejadores para la gráfica decorada
const handleDecoratedChartReady = (chartConfig) => {
  console.log('✅ Gráfica decorada lista:', chartConfig)
  addNotification({
    message: 'Gráfica con patrón decorador cargada exitosamente',
    type: 'success',
    duration: 3000
  })
}
...
const handleExportComplete = (exportResult) => {
  console.log('✅ Exportación completada:', exportResult)
  addNotification({
    message: `Gráfica exportada como ${exportResult.format.toUpperCase()}: ${exportResult.filename}`,
    type: 'success',
    duration: 4000
  })
}

const handleDecoratedChartError = (errorMessage) => {
  console.error('❌ Error en gráfica decorada:', errorMessage)
  addNotification({
    message: `Error en la gráfica decorada: ${errorMessage}`,
    type: 'error',
    duration: 5000
  })
}
...
const pieChartSemesterOriginal = ref('') // Para la gráfica original
...
// Computado para la gráfica original (usa su propio selector)
const filteredForPieChartOriginal = computed(() => {
  if (!pieChartSemesterOriginal.value) return filteredStudents.value
  return filteredStudents.value.filter(s => s.semestre_actual == pieChartSemesterOriginal.value)
})
...
// Datos para el gráfico de pastel (decorado)
...
// Datos para el gráfico de pastel original (mantiene formato original)
const pieChartDataOriginal = computed(() => {
  const total = filteredForPieChartOriginal.value.length
  
  if (total === 0) {
    return []
  }
  
  const approved = filteredForPieChartOriginal.value.filter(student => {
    const promedio = parseFloat(student.promedio_general || 0)
    return promedio >= 70
  }).length
  
  const failed = filteredForPieChartOriginal.value.filter(student => {
    const promedio = parseFloat(student.promedio_general || 0)
    return promedio < 70 && promedio > 0
  }).length
  
  const atRisk = filteredForPieChartOriginal.value.filter(student => 
    student.estatus === 'Baja temporal' || student.estatus === 'Baja definitiva'
  ).length
